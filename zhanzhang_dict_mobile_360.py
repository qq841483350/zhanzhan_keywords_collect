#coding:utf8
#站长工具360移动关键词收录（只提取有指数且有排名关键词）  使用方法，输入某网站网址，即可提取网站所有有排名关键词数据并自动导入数据库
...
def GetHtml(url):
    ...
    post_data=urllib.urlencode(post_data)
    urlNew="http://rank.chinaz.com/rank360/%s-0---0-1"%url   #关键词权重查询页面URL
    req=urllib2.Request(url=urlNew,data=post_data,headers=header)
    html=urllib2.urlopen(req).read()
    return html
def get_mobile_keywords_360(url):
    post_data={"sortType":"0","host":"%s"%url}
    post_data=urllib.urlencode(post_data)
    html=GetHtml(url)
    if 'class="col-blue02' in html:
        if 'col-red lh30 fz14 tc' not in html:
            ...
            for num in range(1,nums):  #循环打开每一页
                url_num="http://rank.chinaz.com/rank360/%s-0---0-%s"%(url,num)
                if num<=1:
                    try:
                        req=urllib2.Request(url=url_num,data=post_data,headers=header)
                        html=urllib2.urlopen(req).read()
                        keywords=re.findall('class="ellipsis block">([\s\S]*?)</a></div><div class="w15-0',html)

                        if keywords:
                            indexs=re.findall('<a href="http://index.chinaz.com/\?words=[\s\S]*?#haosou-module" target="_blank">(\d+)</a></div><div class="w15-0">',html)
                            for x  in range(0,len(keywords)):
                                # print '关键词：',keywords[x],'360移动指数：',indexs[x]
                                try:
                                    cursor.execute("insert into fangdict(word,360_m_index) VALUES ('%s',%s)"%(keywords[x],indexs[x]))
                                    print url,'[360移动]关键词：',keywords[x],'---已成功添加到数据库----'
                                    time.sleep(10)
                                    conn.commit()
                                except:
                                    print url,'[360移动]关键词：',keywords[x],'*******在数据库中已存在********'
                                    continue
